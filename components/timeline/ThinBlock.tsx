...
interface ThinBlockProps {
  ...
  leftOffset: number;
  onPress?: () => void;
  targetBg?:  string; // bg  color of the activity block below, for background gradient
  targetInk?: string; // ink color of the activity block below, for accent bar gradient
}
...
export function ThinBlock({ event, hourHeight, leftOffset, onPress, targetBg, targetInk }: ThinBlockProps) {
  const c = CAT[event.cat];
  const top = event.end * hourHeight - PILL_H + OVERLAP;
  const bgColors:  [string, string] = [c.bg, targetBg  ?? c.bg];
