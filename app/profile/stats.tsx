...
function CategoryBar({ cat, hours, max }: { cat: CatKey; hours: number; max: number }) {
  const C = useColors();
  const barStyles = makeBarStyles(C);
  const c = CAT[cat] ?? { bg: C.surfaceSunk, ink: C.ink3 };
  const filled = max > 0 ? hours / max : 0;
  return (
    <View style={barStyles.row}>
      <View style={[barStyles.dot, { backgroundColor: c.ink }]} />
      <Text style={barStyles.label}>{CAT_LABELS[cat] ?? cat}</Text>
      <View style={barStyles.track}>
        <View style={{ flex: filled, height: 8, backgroundColor: c.ink, borderRadius: 4 }} />
        <View style={{ flex: 1 - filled }} />
      </View>
      <Text style={barStyles.value}>{fmtH(hours)}/sem</Text>
    </View>
  );
}
...
function DayBar({ day, hours, max }: { day: WeekDay; hours: number; max: number }) {
  ...
  return (
    <View style={barStyles.row}>
      <Text style={[barStyles.label, { width: 30 }]}>{DAY_LABELS[day]}</Text>
      <View style={barStyles.track}>
        <View style={{ flex: filled, height: 8, backgroundColor: C.primary, borderRadius: 4 }} />
        <View style={{ flex: 1 - filled }} />
      </View>
      <Text style={barStyles.value}>{fmtH(hours)}</Text>
    </View>
  );
}

function makeBarStyles(C: ReturnType<typeof useColors>) {
  return StyleSheet.create({
    row:   { flexDirection: 'row', alignItems: 'center', gap: Spacing.sm, paddingVertical: 5 },
    dot:   { width: 8, height: 8, borderRadius: 4, flexShrink: 0 },
    label: { fontSize: FontSize.sm, fontWeight: '600', color: C.ink2, width: 100 },
    track: { flex: 1, height: 8, borderRadius: 4, flexDirection: 'row', overflow: 'hidden' },
    value: { fontSize: FontSize.sm, fontWeight: '600', color: C.ink3, width: 60, textAlign: 'right' },
  });
}

function InfoRow({ label, value, last }: { label: string; value: string; last?: boolean }) {
  const C = useColors();
  const infoStyles = makeInfoStyles(C);
  return (
...
function makeInfoStyles(C: ReturnType<typeof useColors>) {
  return StyleSheet.create({
    row:    { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingVertical: Spacing.base },
    border: { borderBottomWidth: 1, borderBottomColor: C.hairline },
    label:  { fontSize: FontSize.base, fontWeight: '600', color: C.ink2 },
    value:  { fontSize: FontSize.base, fontWeight: '700', color: C.ink },
  });
}
...
export default function StatsScreen() {
  ...
  const totalActivityH = weeklyByCategory.reduce((acc, [, h]) => acc + h, 0);
    ...
    <SafeAreaView style={s.safe} edges={['top']}>
      ...
      >
        ...
        {weeklyByCategory.length > 0 ? (
          <>
            <Text style={s.sectionLabel}>Répartition hebdomadaire</Text>
            <View style={s.card}>
          ...
          <>
            <Text style={s.sectionLabel}>Répartition hebdomadaire</Text>
            <View style={s.card}>
              <Text style={s.empty}>Aucune activité planifiée pour l'instant.</Text>
            </View>
          </>
        )}

        {/* ── Charge par jour ────────────────────────────────────────────────── */}
        <Text style={s.sectionLabel}>Semaine type</Text>
        <View style={s.card}>
          {WEEK_DAYS.map((day) => (
        ...
        {/* ── Sommeil ───────────────────────────────────────────────────────── */}
        <Text style={s.sectionLabel}>Sommeil & Routine</Text>
        <View style={s.card}>
        ...
        {/* ── Repas ─────────────────────────────────────────────────────────── */}
        <Text style={s.sectionLabel}>Repas</Text>
        <View style={s.card}>
...
function makeStyles(C: ReturnType<typeof useColors>) {
  return StyleSheet.create({
    ...
    empty: { fontSize: FontSize.base, color: C.ink3, textAlign: 'center', paddingVertical: Spacing.lg },
  });
}
